...
def main():
    ...
    for i, tid in enumerate(unique_ids):
        ...
        try:
            ...
            report_run_issue(
                "bjkc-tie", "parse tie", exc, severity="partial",
    ...
    final_df['_canonical_key'] = final_df.apply(
        lambda row: source_match_key(row.to_dict(), "bjkc"), axis=1
    )
    ...
    if os.path.exists(csv_filename):
        ...
        existing_df['_canonical_key'] = existing_df.apply(
            lambda row: source_match_key(row.to_dict(), "bjkc"), axis=1
        )
        ...
        combined_df = pd.concat(
            [existing_df[~existing_df['_canonical_key'].isin(incoming_ids)], final_df],
            ignore_index=True
        )
