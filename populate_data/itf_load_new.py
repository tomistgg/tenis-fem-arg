...
def _wait_for_itf_drawsheet_request_slot():
    ...
    now = time.monotonic()
    wait_seconds = ITF_DRAWSHEET_REQUEST_INTERVAL_SECONDS - (now - _LAST_ITF_DRAWSHEET_REQUEST_AT)
    if wait_seconds > 0:
...
def _itf_draw_polling_open(start_date_str, today=None):
    ...
    return not (tournament_monday > current_monday and today.weekday() < 5)
...
def _filter_tournaments_with_possible_arg_draws(tournaments_df, cached_ids):
    ...
    skip_mask = tournaments_df.apply(
        lambda row: (
            ...
            and key_to_id.get(str(row.get("tournamentKey") or "").strip().lower(), "") in definitive_no_arg_ids
        ),
        axis=1,
    )
...
def _warn_unresolved_tournament_ids(tournaments_df):
    ...
    if unresolved_ids:
        logger.debug(f"  [i] Skipping {unresolved_ids} ITF tournament(s) with no source ID.")
...
def fetch_api_data(tId, classification, week_number=0, driver=None, tournament_name=""):
    ...
    for attempt in range(2):
        try:
            ...
            raw = response.text.strip()
            blocked_response = response.status_code in {403, 429} or _is_itf_block_page(raw)
            if blocked_response:
                if attempt == 0:
                    logger.warning(
            ...
            if response.status_code == 200 and raw and not raw.startswith("<"):
                ...
                if stale_cached is not None:
                    ...
                    _record_draw_outcome(
                        ...
                        tournament_name,
                        severity="degraded",
                        reason="using stale draw after empty live payload",
                    )
                    return stale_cached
                if isinstance(data, dict):
                    _record_draw_outcome(
                        ...
                        tournament_name,
                        severity="degraded",
                        reason="live draw payload was incomplete",
                    )
    ...
    if blocked_response:
        ...
        if stale_cached is not None:
            ...
            _record_draw_outcome(
                ...
                tournament_name,
                severity="degraded",
                reason="using stale draw after HTTP block",
            )
            return stale_cached
        _record_draw_outcome(
            tId,
            ...
            tournament_name,
            severity="degraded",
            reason="drawsheet blocked after throttled retry with no cached fallback",
        )
        return _ITF_FETCH_BLOCKED
    ...
    if stale_cached is not None:
        ...
        _record_draw_outcome(
            ...
            tournament_name,
            severity="degraded",
            reason="using stale draw after live fetch failure",
        )
        return stale_cached
    ...
    _record_draw_outcome(
        ...
        tournament_name,
        severity="degraded",
        reason="drawsheet fetch failed with no cached fallback",
    )
    return None
...
def parse_drawsheet(data, tourney_meta, draw_type, week_offset=0):
    ...
    def get_p(t):
        ps = t.get("players", [])
        if not ps or not isinstance(ps[0], dict):
            return "Unknown", "", ""
        p = ps[0]
        return (
            p.get("playerId", ""),
            ...
            for match in matches:
                try:
                    ...
                    if match.get("resultStatusCode") == "BYE":
                        ...
                        l_n = "Bye"
                        l_c = "-"
                    elif match.get("resultStatusCode") == "WO" or not any(char.isdigit() for char in res):
                        res = "W/O"
...
def update_csv_smart(filename, new_data_df, reset_if_not_current_week=False, current_week_start=None):
    ...
    if not existing_df.empty:
        existing_ids = set(existing_df.apply(lambda row: source_match_key(row.to_dict(), "itf"), axis=1))
        new_data_df = new_data_df.copy()
        new_data_df["_canonical_key"] = new_data_df.apply(lambda row: source_match_key(row.to_dict(), "itf"), axis=1)
        unique_new_rows = new_data_df[~new_data_df["_canonical_key"].isin(existing_ids)].drop_duplicates(
        ...
        final_df["_canonical_key"] = final_df.apply(lambda row: source_match_key(row.to_dict(), "itf"), axis=1)
...
if __name__ == "__main__":
    ...
    try:
        ...
        if not cached_items:
            ...
            live_items = get_itf_calendar_for_range(
                window_start.strftime("%Y-%m-%d"), window_end.strftime("%Y-%m-%d"), driver=driver
            )
        ...
        if skipped_future_week:
            logger.debug(
                f"  Skipping {skipped_future_week} next-week tournament(s) until the weekend before they start."
            )
        if tournaments_df.empty:
        ...
        if missing_id_mask.any() and cached_ids:
            logger.warning(
                f"[!] {missing_id_mask.sum()} tournament(s) missing IDs from live fetch; filling from cache."
            )
        ...
        regular_ids = {
            ...
            if row.get("category") != "ITF Womens Multi-Week Circuit" and str(row.get("tournamentId") or "").strip()
        }
        ...
        for group_idx, week_group in enumerate(week_groups):
            if group_idx > 0:
                logger.debug("  Sleeping 35s before next week's tournaments...")
                time.sleep(35)
            ...
            for tourney in week_group:
                ...
                if not requested_codes:
                    logger.debug(f"  Skipping {tName} — its published draws contain no ARG players")
                    continue
    ...
    if all_matches:
        added_players = sync_itf_players(Path(DATA_DIR) / "player_aliases_wta_itf.json", all_matches)
        if added_players:
        ...
        update_csv_smart("itf_matches_arg.csv", new_matches_df, reset_if_not_current_week=False)
