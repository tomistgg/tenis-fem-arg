            ...
            window.switchTab = function(tabName) {
                ...
                try {
                    ...
                    if (desired && current !== desired) {
                        ...
                        history.replaceState(null, '', desired || current);
                    }
                } catch (e) {}
            };
...
"""
    ...
    <head>
        ...
        <style>
            ...
            .gallery-back-btn {{ padding: 8px 12px; background: white; border: 1px solid #cbd5e1; border-radius: 8px; font-family: inherit; font-size: 12px; cursor: pointer; }}
            ...
            .gallery-card {{ background: white; border: 1px solid #cbd5e1; border-radius: 8px; overflow: hidden; cursor: pointer; transition: transform 0.15s, box-shadow 0.15s; }}
            ...
            input:focus, select:focus {{ outline: none; border-color: #75AADB; box-shadow: 0 0 0 3px rgba(117,170,219,0.2); }}
            ...
            .content-card {{ background: white; box-shadow: 0 4px 20px rgba(0,0,0,0.05); width: 100%; border: 1px solid #cbd5e1; }}
            .table-wrapper {{ overflow-x: auto; width: 100%; }}
            table {{ border-collapse: separate; border-spacing: 0; width: 100%; table-layout: fixed; border: 1px solid #cbd5e1; }}
            #view-upcoming table {{ width: max-content; min-width: 100%; }}
            th {{ position: sticky; top: 0; background: linear-gradient(180deg, #75AADB 0%, #4d89c3 100%) !important; color: white; padding: 10px 15px; font-size: 11px; font-weight: bold; border-bottom: 2px solid rgba(0,0,0,0.25); border-right: 1px solid rgba(255,255,255,0.25); z-index: 10; text-transform: uppercase; text-align: center; }}
            td {{ padding: 8px 12px; border-bottom: 1px solid #cbd5e1; text-align: center; font-size: 13px; border-right: 1px solid #cbd5e1; }}
            ...
            .entry-menu {{ width: 480px; flex-shrink: 0; display: flex; flex-wrap: wrap; align-items: stretch; background: white; border: 1px solid #cbd5e1; align-self: flex-start; }}
            ...
            @media (max-width: 480px) {{
                ...
                /* Upcoming search: narrower and shorter */
                #view-upcoming .search-container {{ width: auto; }}
                #view-upcoming #s {{ width: 130px; max-width: 130px; padding: 4px 8px; font-size: 10px; }}

                /* Rankings: Show ARG slightly left, date picker same height as button */
                #view-rankings .rankings-controls {{ padding-right: 8px; }}
                #view-rankings .rankings-toggle-btn, #view-rankings .rankings-date-picker {{ height: 30px; box-sizing: border-box; }}
            }}
...
"""
    ...
    for tab in route_tabs:
        folder = os.path.join(os.path.dirname(__file__), tab)
