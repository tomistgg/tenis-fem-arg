...
def generate_html(tournament_groups, tournament_store, players_data, schedule_map,
    ...
    photos_manifest_path = os.path.join(RUNTIME_DATA_DIR, 'photos_by_player.json')
    try:
        with open(photos_manifest_path, 'r', encoding='utf-8') as f:
            photos_by_player_manifest = json.load(f)
    except (OSError, json.JSONDecodeError):
        photos_by_player_manifest = {}
...
<body>
  <div class="home-hero">
    ...
    <div class="home-grid">
      ...
      <a class="home-btn" href="app.html#gallery"><img class="home-icon-img" src="assets/camera.png" alt="Camera icon"><span class="home-label">Photos</span></a>
...
"""
    ...
    <head>
        ...
        <style>
            ...
            .rankings-load-btn:hover,
            .gallery-loadmore-btn:hover {{ background: var(--c-primary-hover); }}
            ...
            *:focus-visible {{ outline: 2px solid var(--c-focus-ring); outline-offset: 2px; }}
            ...
            #view-gallery {{ max-width: 1400px; margin: 0 auto; align-items: center; }}
            ...
            .gallery-controls {{ display: flex; gap: 10px; flex-wrap: wrap; align-items: center; justify-content: center; margin: 0 auto 12px; width: min(100%, 1120px); }}
            ...
            .gallery-back-btn {{ border-width: 1px; }}
            ...
            .gallery-album-card {{ background: white; border: 1px solid #cbd5e1; border-radius: 10px; overflow: hidden; cursor: pointer; transition: transform 0.15s, box-shadow 0.15s; }}
            ...
            .gallery-wall {{ width: 100%; display: flex; justify-content: center; }}
            .gallery-grid {{ width: min(100%, 1100px); height: min(76vh, 760px); display: grid; grid-template-columns: repeat(12, 1fr); grid-template-rows: repeat(12, 1fr); gap: 4px; }}
            .gallery-card {{ width: 100%; height: 100%; overflow: hidden; background: #e2e8f0; border: 0; padding: 0; cursor: pointer; display: block; }}
            .gallery-card:hover {{ box-shadow: 0 8px 24px rgba(0,0,0,0.13); }}
            .gallery-card img {{ width: 100%; height: 100%; display: block; object-fit: cover; background: #e2e8f0; }}
            ...
            .photos-removal-notice {{ width: min(100%, 720px); margin: clamp(36px, 9vh, 88px) auto 0; padding: clamp(26px, 5vw, 42px); box-sizing: border-box; border: 1px solid var(--c-border); border-top: 4px solid var(--c-primary); border-radius: var(--radius-xl); background: var(--c-surface); box-shadow: var(--shadow-md); color: var(--c-text-secondary); font-size: var(--fs-2xl); line-height: 1.75; text-align: center; }}
            .photos-removal-notice p {{ margin: 0; }}
            .photos-removal-notice p + p {{ margin-top: var(--sp-6); padding-top: var(--sp-5); border-top: 1px solid var(--c-border-soft); color: var(--c-text-primary); font-weight: 800; }}
            ...
            @media (max-width: 768px) {{
                ...
                .gallery-grid {{ width: min(100%, 1100px); height: min(66vh, 520px); gap: 3px; grid-template-columns: repeat(12, 1fr); grid-template-rows: repeat(12, 1fr); }}
            ...
            [data-theme="dark"] .photos-removal-notice {{ background: var(--c-surface); border-color: var(--c-border); border-top-color: var(--c-primary); color: var(--c-text-secondary); }}
            ...
            @media (max-width: 768px) {{
                ...
                .gallery-albums {{ gap: 8px; }}
                .gallery-album-card {{ border-radius: 12px; }}
            }}
    ...
    <body class="home-mode" onload="renderHistoryTable(); renderMilestonesTable();">
        ...
        <div class="app-container">
            <nav class="sidebar" id="sidebar" aria-label="Primary">
                ...
                <button type="button" class="menu-item" id="btn-gallery" onclick="switchTab('gallery')">Photos</button>
            </nav>
                    ...
                    <div class="home-hero">
                        ...
                        <div class="home-grid">
                            <button class="home-btn" onclick="switchTab('gallery')">
                                ...
                                <span class="home-label">Photos</span>
                            </button>
                ...
                <div id="view-roadtogs" class="single-layout" style="display: none;">
                    ...
                    <div class="roadtogs-meta">
                        ...
                        <div class="roadtogs-meta-col roadtogs-meta-cutoffs">
                            ...
                            <div class="roadtogs-meta-line">Cutoffs 2026 (Q/MD) = <span style="color:#0066B3;font-weight:700;">AO 308/754</span> - <span style="color:#C8602A;font-weight:700;">RG 283/786</span> - <span style="color:#3D7A3D;font-weight:700;">WB 315/736</span></div>
                        </div>
                    </div>
                    <div class="content-card">
                        <div class="table-wrapper">
        ...
        <script>
            ...
            function collectGalleryUrlState() {{
                const select = document.getElementById('gallery-player-filter');
                return select && select.value ? {{ player: select.value }} : {{}};
            }}

            function restoreGalleryUrlState(params) {{
                if (!params.has('player')) return;
                const select = document.getElementById('gallery-player-filter');
                if (!select || !setSelectValueFromSlug(select, params.get('player'))) return;
                renderManualPhotos();
            ...
            function collectUrlStateForTab(tabName) {{
                switch (tabName) {{
                    ...
                    case 'gallery': return collectGalleryUrlState();
                    default: return {{}};
                }}
            }}
            ...
            async function restoreUrlStateForTab(tabName) {{
                ...
                try {{
                    switch (tabName) {{
                        ...
                        case 'gallery': restoreGalleryUrlState(params); break;
                    }}
                }} finally {{
            ...
            // ---- Photos ---------------------------------------------------
            const PHOTOS_BY_PLAYER_PATH = 'data/photos_by_player.json';
            const PHOTOS_BY_PLAYER_INLINE = {_script_safe_json(photos_by_player_manifest, ensure_ascii=False, separators=(',', ':'))};
            let photosByPlayer = {{}};
            ...
            function normalizePhotoSlots(slots) {{
                ...
                for (let i = 0; i < 12; i++) {{
                    ...
                    out.push(value ? String(value).trim() : '');
                }}
                return out;
            }}

            function resolvePhotoUrl(playerName, value) {{
                const text = String(value || '').trim();
                if (!text) return '';
                if (text.startsWith('photos/') || text.startsWith('http://') || text.startsWith('https://')) return text;
                return 'photos/' + playerName + '/' + text;
            }}

            function normalizePhotosManifest(data) {{
                const out = {{}};
                if (!data || typeof data !== 'object' || Array.isArray(data)) return out;
                Object.keys(data).sort().forEach(function(playerName) {{
                    const rawSlots = Array.isArray(data[playerName]) ? data[playerName] : [];
                    out[playerName] = normalizePhotoSlots(rawSlots.map(function(slot) {{
                        return resolvePhotoUrl(playerName, slot);
                    }}));
                }});
                return out;
            }}

            let galleryLightboxPhotoIndex = -1;
            let galleryLightboxPhotos = [];
            let galleryLightboxReturnFocus = null;
            let galleryLightboxPreviousOverflow = '';

            const GALLERY_COLLAGE_LAYOUTS_DESKTOP = {{
                1: [{{ col: '1 / 13', row: '1 / 13' }}],
                2: [
                    {{ col: '1 / 7', row: '1 / 13' }},
                    {{ col: '7 / 13', row: '1 / 13' }}
            ...
            const GALLERY_COLLAGE_LAYOUTS_MOBILE = {{
                1: [{{ col: '1 / 13', row: '1 / 5' }}],
                2: [
                    {{ col: '1 / 7', row: '1 / 13' }},
                    {{ col: '7 / 13', row: '1 / 13' }}
            ...
            function getGalleryCollageLayout(count) {{
                const isMobile = window.matchMedia && window.matchMedia('(max-width: 700px)').matches;
                if (isMobile) {{
                    const layouts = GALLERY_COLLAGE_LAYOUTS_MOBILE;
                    return layouts[count] || layouts[12] || layouts[10] || [];
                }}
                const displayCount = Math.max(1, Math.min(count, 12));
                const rowCount = Math.min(3, Math.ceil(displayCount / 4));
                const rowCounts = [];
                let remaining = displayCount;
                for (let rowIdx = 0; rowIdx < rowCount; rowIdx++) {{
                    const rowsLeft = rowCount - rowIdx;
                    const cardsThisRow = Math.ceil(remaining / rowsLeft);
                    rowCounts.push(cardsThisRow);
                    remaining -= cardsThisRow;
                }}
                const layout = [];
                let rowStart = 1;
                rowCounts.forEach(function(cardsThisRow) {{
                    const rowEnd = rowStart + (12 / rowCount);
                    const colSpan = 12 / cardsThisRow;
                    for (let colIdx = 0; colIdx < cardsThisRow; colIdx++) {{
                        layout.push({{
                            col: `${{1 + colIdx * colSpan}} / ${{1 + (colIdx + 1) * colSpan}}`,
                            row: `${{rowStart}} / ${{rowEnd}}`
                        }});
                    }}
                    rowStart = rowEnd;
            ...
            function openGalleryLightbox(url, altText, photoIndex, photos, triggerElement) {{
                const lightbox = document.getElementById('gallery-lightbox');
                const image = document.getElementById('gallery-lb-img');
                if (!lightbox || !image) return;
                galleryLightboxReturnFocus = triggerElement || document.activeElement;
                galleryLightboxPreviousOverflow = document.body.style.overflow || '';
                galleryLightboxPhotoIndex = typeof photoIndex === 'number' ? photoIndex : -1;
                galleryLightboxPhotos = Array.isArray(photos) ? photos.slice() : [];
                image.src = url;
                image.alt = altText || '';
                ...
                const initialFocus = lightbox.querySelector('.gallery-lb-close') || lightbox;
            ...
            function closeGalleryLightbox(event) {{
                ...
                }} else if (lightbox && lightbox.contains(document.activeElement)) {{
                ...
                if (image) image.src = '';
                document.body.style.overflow = galleryLightboxPreviousOverflow;
                document.removeEventListener('keydown', handleGalleryLightboxKeydown);
                galleryLightboxPhotoIndex = -1;
                galleryLightboxPhotos = [];
                galleryLightboxPreviousOverflow = '';
            }}

            function populatePhotoPlayers() {{
                const select = document.getElementById('gallery-player-filter');
                if (!select) return;
                const current = select.value;
                select.innerHTML = '<option value="">Select a player</option>';
                Object.keys(photosByPlayer).sort().forEach(function(playerName) {{
                    const option = document.createElement('option');
                    option.value = playerName;
                    option.textContent = playerName;
                    select.appendChild(option);
                }});
                if (current && photosByPlayer[current]) {{
            ...
            function renderManualPhotos() {{
                ...
                if (!select || !grid || !placeholder) return;

                const playerName = select.value;
                if (!playerName) {{
                    galleryCurrentPhotos = [];
                    grid.innerHTML = '';
                    grid.style.display = 'none';
                    placeholder.style.display = 'block';
                    placeholder.textContent = 'Select a player to show photos';
                    syncUrlStateForTab('gallery');
                    return;
                }}

                const photos = normalizePhotoSlots(photosByPlayer[playerName]).filter(Boolean);
                if (!photos.length) {{
                    galleryCurrentPhotos = [];
                    grid.innerHTML = '';
                    grid.style.display = 'none';
                    placeholder.style.display = 'block';
                    placeholder.textContent = 'No photo URLs added yet for this player.';
                    syncUrlStateForTab('gallery');
                    return;
                }}

                const layout = getGalleryCollageLayout(photos.length);
                galleryCurrentPhotos = photos.slice();
                const cards = photos.map(function(url, idx) {{
                    const slot = layout[idx] || layout[layout.length - 1] || {{ col: 'auto', row: 'auto' }};
                    const slotStyle = ' style="grid-column:' + slot.col + ';grid-row:' + slot.row + ';"';
                    return '<button type="button" class="gallery-card"' + slotStyle + ' data-photo-index="' + idx + '" aria-label="Open ' + galleryEsc(playerName + ' photo ' + (idx + 1)) + '">'
                        + '<img src="' + galleryEsc(url) + '" alt="' + galleryEsc(playerName + ' photo ' + (idx + 1)) + '" loading="lazy" />'
                        + '</button>';
                }});
            ...
            function initGallery() {{
                if (galleryInited || galleryLoading) return;
                galleryLoading = true;
                const inlinePhotos = normalizePhotosManifest(PHOTOS_BY_PLAYER_INLINE);
                const select = document.getElementById('gallery-player-filter');
                const currentSelection = select ? select.value : '';
                photosByPlayer = inlinePhotos;
                populatePhotoPlayers();
                if (select && !select._galleryBound) {{
                    select.addEventListener('change', renderManualPhotos);
                    select._galleryBound = true;
                }}
                const grid = document.getElementById('gallery-grid');
                if (grid && !grid._galleryCardBound) {{
                    grid.addEventListener('click', function(event) {{
                        const card = event.target.closest('.gallery-card');
                        if (!card || !grid.contains(card)) return;
                        const idx = Number.parseInt(card.getAttribute('data-photo-index') || '-1', 10);
                        if (!Number.isInteger(idx) || idx < 0 || idx >= galleryCurrentPhotos.length) return;
                        const playerName = select ? select.value : '';
                        openGalleryLightbox(
                            galleryCurrentPhotos[idx],
                            (playerName || 'Player') + ' photo ' + (idx + 1),
                            idx,
                            galleryCurrentPhotos,
                            card
                        );
                    }});
                    grid._galleryCardBound = true;
                }}
                if (select && currentSelection && photosByPlayer[currentSelection]) {{
                    select.value = currentSelection;
                }}
                if (Object.keys(inlinePhotos).length > 0) {{
                    renderManualPhotos();
                }}
                fetch(PHOTOS_BY_PLAYER_PATH, {{ cache: 'no-store' }})
                    .then(function(r) {{
                        if (!r || !r.ok) throw new Error('photo_manifest_fetch_failed');
                        return r.json();
                    }})
                    .then(function(data) {{
                        const fetched = normalizePhotosManifest(data);
                        if (Object.keys(fetched).length > 0) {{
                            photosByPlayer = fetched;
                            populatePhotoPlayers();
                            if (select && currentSelection && photosByPlayer[currentSelection]) {{
                                select.value = currentSelection;
                            }}
...
{router_script}
    ...
    # photo URLs are handled by the SPA fallback, without per-file redirects.
