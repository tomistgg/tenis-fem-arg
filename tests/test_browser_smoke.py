...
@pytest.mark.browser
def test_entry_withdrawal_toggle_deadline_and_translations(offline_generated_site):
    browser_binary, driver_binary = _browser_paths()
    if not browser_binary or not driver_binary:
        if os.environ.get("WTARG_REQUIRE_BROWSER_TESTS") == "1":
            pytest.fail("Chrome browser/driver not available")
        pytest.skip("Chrome browser/driver not available")
    ...
    try:
        ...
        wait.until(lambda current: current.execute_script("return typeof updateEntryList") == "function")
        driver.execute_script("""
            const fixtures = {
                'https://example.com/wta': {deadline: null, rows: [
                    {position: '1-MD', name: 'Withdrawn <Player>', country: 'ARG', rank: '42', date: '2026-09-16'}
                ]},
                'itf-open': {deadline: '1900-01-01', rows: []},
                'itf-future': {deadline: '9999-01-01', rows: []}
            };
            ...
            switchTab('entrylists');
            selectEntryTournament(menu.querySelector('.entry-menu-item'));
        """)
        button = driver.find_element(By.ID, "btn-prio1")
        assert button.text == "Show Withdrawals"
        button.click()
        assert button.text == "Show E-List"
        ...
        button.click()
        wait.until(lambda _driver: button.text == "Mostrar Bajas")
        assert "Current Player" in driver.find_element(By.ID, "entry-body").text
        ...
        assert button.text == "Show Withdrawals"
        button.click()
        assert "No withdrawals recorded." in driver.find_element(By.ID, "entry-body").text
