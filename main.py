import requests
import time
from datetime import datetime, timedelta
from bs4 import BeautifulSoup

# --- CONFIGURATION ---
TOURNAMENT_GROUPS = {
    "Semana 16 Febrero": {
        "https://www.wtatennis.com/tournaments/dubai/player-list": "WTA 1000 DUBAI",
        "https://www.wtatennis.com/tournaments/2051/midland-125/2026/player-list": "WTA 125 MIDLAND",
        "https://www.wtatennis.com/tournaments/1156/oeiras-125-indoor-2/2026/player-list": "WTA 125 OEIRAS 2",
        "https://www.wtatennis.com/tournaments/1157/les-sables-d-olonne-125/2026/player-list": "WTA 125 LES SABLES",
        "w-itf-ind-2026-006": "W100 BENGALURU",
        "w-itf-ger-2026-002": "W75 ALTENKIRCHEN",
        "w-itf-fra-2026-005": "W50 MACON",
        "w-itf-usa-2026-006": "W35 LAS VEGAS, NV",
        "w-itf-ned-2026-001": "W35 THE HAGUE",
        "w-itf-egy-2026-011": "W15 SHARM ELSHEIKH",
        "w-itf-tun-2026-007": "W15 MONASTIR",
        "w-itf-tur-2026-007": "W15 ANTALYA",
        "w-itf-usa-2026-007": "W15 PALM COAST, FL",
        "w-itf-esp-2026-008": "W15 MANACOR"
    },
    "Semana 23 Febrero": {
        "https://www.wtatennis.com/tournaments/2085/m-rida/2026/player-list": "WTA 500 MERIDA",
        "https://www.wtatennis.com/tournaments/2082/austin/2026/player-list": "WTA 250 AUSTIN",
        "https://www.wtatennis.com/tournaments/1124/antalya-125-1/2026/player-list": "WTA 125 ANTALYA 1",
    },
    "Semana 2 Marzo": {
        "https://www.wtatennis.com/tournaments/609/indian-wells/2026/player-list": "WTA 1000 INDIAN WELLS",
        "https://www.wtatennis.com/tournaments/1107/antalya-125-2/2026/player-list": "WTA 125 ANTALYA 2",
    },
    "Semana 9 Marzo": {
        "https://www.wtatennis.com/tournaments/1161/austin-125-1/2026/player-list": "WTA 125 AUSTIN",
        "https://www.wtatennis.com/tournaments/1125/antalya-125-3/2026/player-list": "WTA 125 ANTALYA 3",
    }
}

API_URL = "https://api.wtatennis.com/tennis/players/ranked"
HEADERS = {"User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/144.0.0.0 Safari/537.36"}

def get_all_rankings(date_str):
    all_players, page = [], 0
    while True:
        params = {"metric": "SINGLES", "type": "rankSingles", "sort": "asc", "at": date_str, "pageSize": 100, "page": page}
        try:
            r = requests.get(API_URL, params=params, headers=HEADERS, timeout=10)
            data = r.json()
            items = data.get('content', []) if isinstance(data, dict) else data
            if not items: break
            all_players.extend(items)
            page += 1
            time.sleep(0.05)
        except: break
    return [{"Player": p.get('player', {}).get('fullName').strip(), "Rank": p.get('ranking'), "Country": p.get('player', {}).get('countryCode', ''), "Key": p.get('player', {}).get('fullName').strip().upper()} for p in all_players if p.get('player')]

def get_itf_players(tournament_key):
    url = f"https://www.itftennis.com/tennis/api/TournamentApi/GetAcceptanceList?tournamentKey={tournament_key}&circuitCode=WT"
    try:
        r = requests.get(url, headers=HEADERS, timeout=15)
        data = r.json()
    except Exception as e:
        print(f"Error en {tournament_key}: {e}")
        return {}

    final_list = {}
    
    for item in data:
        for classification in item.get("entryClassifications", []):
            desc = classification.get("entryClassification", "").upper()
            code = classification.get("entryClassificationCode", "")

            # IGNORAR COMPLETAMENTE SI ES WITHDRAWALS (y lo que sigue)
            if "WITHDRAWAL" in desc:
                break 

            for entry in classification.get("entries", []):
                # Capturamos la posición (ej: "1", "2", etc.)
                pos = entry.get("positionDisplay", "")
                
                # DETERMINAR EL SUFIJO
                if code == "MDA":
                    suffix = ""
                elif code == "ALT" or "ALTERNATE" in desc:
                    # Formato solicitado: (ALT 1), (ALT 2), etc.
                    suffix = f" (ALT {pos})"
                else:
                    suffix = " (Q)"
                
                # 'or []' evita el TypeError si players es null
                for player in (entry.get("players") or []):
                    full_name = f"{player['givenName']} {player['familyName']}".strip().upper()
                    final_list[full_name] = suffix
    
    return final_list

def scrape_tournament_players(url):
    try:
        r = requests.get(url, headers=HEADERS, timeout=15)
        soup = BeautifulSoup(r.text, 'html.parser')
    except: return {}

    main_draw, qualifying = set(), set()
    current_state = "MAIN" 

    for tag in soup.find_all(True):
        text = tag.get_text().strip()
        ui_tab = tag.get('data-ui-tab', '')

        if text == "Doubles" or ui_tab == "Doubles":
            current_state = "IGNORE"
        elif text == "Qualifying" or ui_tab == "Qualifying":
            current_state = "QUAL"
        elif text == "Main Draw" or ui_tab == "Main Draw":
            current_state = "MAIN"

        p_name = tag.get('data-tracking-player-name')
        if p_name:
            name_key = p_name.strip().upper()
            if current_state == "MAIN": main_draw.add(name_key)
            elif current_state == "QUAL": qualifying.add(name_key)

    final_list = {}
    for p in qualifying: final_list[p] = " (Q)"
    for p in main_draw: final_list[p] = ""
    return final_list

def main():
    ranking_date = (datetime.now() - timedelta(days=datetime.now().weekday())).strftime("%Y-%m-%d")
    players_data = get_all_rankings(ranking_date)
    schedule_map = {}
    
    for week, tourneys in TOURNAMENT_GROUPS.items():
        for key, t_name in tourneys.items():
            # Decidimos qué función usar
            if key.startswith("http"):
                status_dict = scrape_tournament_players(key)
            else:
                status_dict = get_itf_players(key)
            
            for p_key, suffix in status_dict.items():
                if p_key not in schedule_map: schedule_map[p_key] = {}
                
                entry = f"{t_name}{suffix}"
                if week in schedule_map[p_key]:
                    if entry not in schedule_map[p_key][week]:
                        schedule_map[p_key][week] += f"<br>{entry}"
                else:
                    schedule_map[p_key][week] = entry

    table_rows = ""
    week_keys = list(TOURNAMENT_GROUPS.keys())
    for p in players_data:
        is_arg = "is-arg" if p['Country'] == "ARG" else "is-other"
        row = f'<tr class="{is_arg}" data-name="{p["Player"].lower()}">'
        row += f'<td class="sticky-col col-rank">{p["Rank"]}</td>'
        row += f'<td class="sticky-col col-name">{p["Player"]}</td>'
        row += f'<td><span class="{"arg-pill" if p["Country"] == "ARG" else ""}">{p["Country"]}</span></td>'
        for week in week_keys:
            val = schedule_map.get(p['Key'], {}).get(week, "—")
            row += f'<td class="col-week">{"<b>" if "(Q)" not in val and val != "—" else ""}{val}{"</b>" if "(Q)" not in val and val != "—" else ""}</td>'
        table_rows += row + "</tr>"

    html_template = f"""
    <!DOCTYPE html>
    <html lang="es">
    <head>
        <meta charset="UTF-8">
        <title>Tenistas Argentinas - Próximos Torneos</title>
        <style>
            @font-face {{ font-family: 'Montserrat'; src: url('Montserrat-SemiBold.ttf'); }}
            
            body {{ 
                font-family: 'Montserrat', sans-serif; 
                background: #f0f4f8; 
                margin: 0; 
                padding: 0; 
                display: flex; 
                height: 100vh;
                overflow: hidden;
            }}
            
            .app-container {{ display: flex; width: 100%; height: 100%; }}
            
            .sidebar {{ 
                width: 260px; 
                background: #1e293b; 
                color: white; 
                display: flex; 
                flex-direction: column;
                box-shadow: 2px 0 10px rgba(0,0,0,0.1);
            }}
            
            .sidebar-header {{ 
                padding: 25px 20px; 
                font-size: 20px; 
                font-weight: 800; 
                color: #75AADB; 
                border-bottom: 1px solid #334155; 
                margin-bottom: 10px; 
            }}
            
            .menu-item {{ 
                padding: 15px 20px; 
                cursor: pointer; 
                transition: all 0.3s; 
                font-size: 14px; 
                color: #cbd5e1;
            }}
            
            .menu-item:hover {{ background: #334155; color: white; }}
            
            .menu-item.active {{ 
                background: #75AADB; 
                color: white; 
                font-weight: bold; 
                border-left: 5px solid #ffffff; 
            }}

            .main-content {{ 
                flex: 1; 
                padding: 40px; 
                overflow-y: auto; 
                background: #f8fafc; 
            }}

            h1 {{ 
                margin-top: 0; 
                margin-bottom: 30px;
                color: #1e293b; 
                font-size: 28px; 
                font-weight: 800;
            }}

            .content-card {{ 
                background: white; 
                border-radius: 12px; 
                box-shadow: 0 4px 20px rgba(0,0,0,0.05);
                overflow: hidden;
                max-width: 1200px;
            }}

            .table-nav {{ 
                display: flex; 
                justify-content: space-between; 
                padding: 20px; 
                background: #ffffff; 
                border-bottom: 1px solid #e2e8f0; 
            }}

            input {{ 
                padding: 10px 15px; 
                border: 1px solid #cbd5e1; 
                border-radius: 8px; 
                width: 280px; 
                font-family: 'Montserrat';
            }}
            
            #toggleBtn {{
                background: linear-gradient(135deg, #87CEEB 0%, #75AADB 100%);
                color: white; border: none; padding: 10px 25px; 
                font-size: 11px; font-weight: 700; border-radius: 25px; 
                cursor: pointer; text-transform: uppercase; 
                box-shadow: 0 4px 12px rgba(117, 170, 219, 0.3);
            }}

            .table-wrapper {{ overflow-x: auto; max-height: 70vh; }}
            table {{ width: 100%; border-collapse: separate; border-spacing: 0; }}
            th {{ position: sticky; top: 0; background: white; z-index: 10; padding: 15px 10px; border-bottom: 2px solid #e2e8f0; border-right: 1px solid #e2e8f0; font-size: 11px; color: #1e293b; text-transform: uppercase; font-weight: 800; opacity: 1 !important; }}
            td {{ padding: 12px 10px; border-bottom: 1px solid #f1f5f9; border-right: 1px solid #f1f5f9; text-align: center; font-size: 13px; vertical-align: middle; line-height: 1.4; }}
            .sticky-col {{ position: sticky; background: white !important; z-index: 5; border-right: 1px solid #e2e8f0; border-right: 1px solid #e2e8f0 !important; }}
            .col-rank {{ left: 0; width: 60px; }}
            .col-name {{ left: 60px; width: 200px; font-weight: bold; text-align: left; }}
            .arg-pill {{ color: #75AADB; font-weight: 800; border-bottom: 2px solid #75AADB; }}
            tr.hidden {{ display: none; }}
            .placeholder-text {{ padding: 40px; color: #94a3b8; font-style: italic; text-align: center; }}

            th:last-child, td:last-child {{ border-right: none; }}
        </style>
    </head>
    <body>
    <div class="app-container">
        <div class="sidebar">
            <div class="sidebar-header">Tenistas Argentinas</div>
            <div class="menu-item active" onclick="switchTab('Próximos Torneos', this)">Próximos Torneos</div>
            <div class="menu-item" onclick="switchTab('Calendario', this)">Calendario</div>
            <div class="menu-item" onclick="switchTab('Ranking', this)">Ranking</div>
        </div>
        <div class="main-content">
            <h1 id="viewTitle">Próximos Torneos</h1>
            <div id="torneosView" class="tab-content">
                <div class="content-card">
                    <div class="table-nav">
                        <input type="text" id="s" placeholder="Buscar jugadora..." oninput="filter()">
                        <button id="toggleBtn" onclick="toggleMode()">Mostrar Todas</button>
                    </div>
                    <div class="table-wrapper">
                        <table>
                            <thead>
                                <tr>
                                    <th class="sticky-col col-rank">Rank</th>
                                    <th class="sticky-col col-name">Jugadora</th>
                                    <th class="col-country">País</th>
                                    {"".join([f'<th class="col-week">{w}</th>' for w in week_keys])}
                                </tr>
                            </thead>
                            <tbody id="tb">{table_rows}</tbody>
                        </table>
                    </div>
                </div>
            </div>
            <div id="placeholderView" class="tab-content" style="display:none;">
                <div class="content-card"><p class="placeholder-text">Contenido no disponible en este momento.</p></div>
            </div>
        </div>
    </div>
    <script>
        let mode = 'arg';
        function switchTab(tabName, element) {{
            document.getElementById('viewTitle').innerText = tabName;
            document.querySelectorAll('.menu-item').forEach(i => i.classList.remove('active'));
            element.classList.add('active');
            if (tabName === 'Próximos Torneos') {{
                document.getElementById('torneosView').style.display = 'block';
                document.getElementById('placeholderView').style.display = 'none';
            }} else {{
                document.getElementById('torneosView').style.display = 'none';
                document.getElementById('placeholderView').style.display = 'block';
            }}
        }}
        function toggleMode() {{
            const btn = document.getElementById('toggleBtn');
            if (mode === 'all') {{
                mode = 'arg';
                btn.innerText = "Mostrar Todas";
            }} else {{
                mode = 'all';
                btn.innerText = "Mostrar ARG";
            }}
            filter();
        }}
        function filter() {{
            const q = document.getElementById('s').value.toLowerCase();
            document.querySelectorAll('#tb tr').forEach(row => {{
                const matches = row.getAttribute('data-name').includes(q);
                const isArg = row.classList.contains('is-arg');
                row.classList.toggle('hidden', !(matches && (mode === 'all' || isArg)));
            }});
        }}
        window.onload = filter;
    </script>
    </body>
    </html>
    """
    with open("index.html", "w", encoding="utf-8") as f: f.write(html_template)

if __name__ == "__main__": main()