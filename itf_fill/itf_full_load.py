...
if __name__ == "__main__":
    for year in range(1998, 1995, -1):
        ...
        if not raw_data:
            print(f"No calendar data found for {year}. Skipping to next year.")
            continue

        tournaments_df = create_tournament_df(raw_data)
        
        if tournaments_df is not None and not tournaments_df.empty:
            ...
            if all_matches:
                ...
                file_path = os.path.join(script_dir, f"itf_matches_{year}.csv")
                
                final_matches_df.to_csv(file_path, index=False, encoding='utf-8-sig')
                print(f"\nSUCCESS! Saved {len(final_matches_df)} matches to:\n{file_path}")
            else:
                print(f"\nFinished {year}, but no matches were found.")
        else:
