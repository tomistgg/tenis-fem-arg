...
from utils import (
    save_json_file, dumps_itf_calendar_cache,
    expand_itf_calendar_cache, compress_itf_rankings_cache,
    expand_itf_rankings_cache,
    get_cache_timestamp, make_data_status, set_cache_file_meta, utc_now_iso,
)
...
def _number_duplicate_itf_names(items, *, source_field, target_field, sort_key=None):
    ...
    if sort_key is None:
        sort_key = lambda item: item.get("startDate", "") if isinstance(item, dict) else ""

    name_counts = {}
    for item in items:
        if not isinstance(item, dict):
            continue
        name = item.get(source_field, "")
...
def parse_itf_entry_list(itf_entries):
    ...
    for classification in itf_entries:
        ...
        for entry in classification.get("entries") or []:
            ...
            player_id = str(
                p_node.get("playerId")
                or p_node.get("playerID")
                or p_node.get("id")
                or p_node.get("player_id")
                or ""
            ).strip()
            display_name = resolve_player_display_name(
                "itf", player_id=player_id, name=raw_f_name
            )
            ...
            players.append({
                "pos": pos, "name": display_name, "country": p_node.get("nationalityCode", "-"),
...
def _is_invalid_browser_session(exc):
    return (
        ...
        or "invalid session id" in str(exc).lower()
    )
...
def _build_name_map(entry_classifications):
    ...
    for classification in entry_classifications or []:
        ...
        for entry in classification.get("entries") or []:
            pos = entry.get("positionDisplay", "")
            suffix = "" if code in ("MDA", "CA", "JR", "JA", "SE", "WC") else (f" (ALT {pos})" if code in ("ALT", "A") or "ALTERNATE" in desc else " (Q)")
            ...
            for p in players:
                ...
                matched_name = resolve_player_display_name(
                    "itf", player_id=player_id, name=full_name
                ).upper()
        ...
        try:
            ...
            if all_players:
                raise ItfApiPartialData(
                    f"ITF rankings fetch interrupted after {len(all_players)} players "
                    f"for {nationality}: {e}"
                )
            raise ItfApiFetchError(f"ITF rankings fetch failed for {nationality}: {e}")

    if expected_total is not None and len(all_players) < expected_total:
        raise ItfApiPartialData(
            f"ITF rankings fetch ended early after {len(all_players)}/{expected_total} players "
            f"for {nationality}"
        )
...
def _load_itf_rankings_cache(*, strict=False):
    if not os.path.exists(ITF_CACHE_FILE):
        return {}
    try:
        with open(ITF_CACHE_FILE, "r", encoding="utf-8") as f:
...
def get_itf_rankings_cached(date_str, nationality="ARG", *, with_status=False):
    ...
    if new_data:
        ...
        return _finish(new_data, make_data_status(
            "ITF rankings",
            "fresh",
            requested=date_str,
            effective=date_str,
    ...
    reason = (
        ...
        if fetch_error else
        "No ITF rankings were available for the requested date."
    )
